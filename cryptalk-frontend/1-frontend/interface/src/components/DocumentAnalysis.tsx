...
const DocumentAnalysis: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/client/${clientId}/upload`, {
        method: 'POST',
        body: formData,
      });
        ...
        <ModalContent>
          ...
          <ModalBody>
            <VStack spacing={4}>
              ...
              <FormControl>
                <FormLabel mb={3}>Tipo de Documento</FormLabel>
                <VStack spacing={3}>
                  <Button
                    size="md"
                    width="100%"
                    variant={documentType === 'pitch-deck' ? 'solid' : 'outline'}
                    colorScheme={documentType === 'pitch-deck' ? 'blue' : 'gray'}
                    onClick={() => setDocumentType('pitch-deck')}
                    leftIcon={<Text>📊</Text>}
                  >
                    Pitch Deck (PPT/PDF)
                  </Button>
                  <Button
                    size="md"
                    width="100%"
                    variant={documentType === 'patent' ? 'solid' : 'outline'}
                    colorScheme={documentType === 'patent' ? 'blue' : 'gray'}
                    onClick={() => setDocumentType('patent')}
                    leftIcon={<Text>📄</Text>}
                  >
                    Patent (DOC/PDF)
                  </Button>
                  <Button
                    size="md"
                    width="100%"
                    variant={documentType === 'financial-projection' ? 'solid' : 'outline'}
                    colorScheme={documentType === 'financial-projection' ? 'blue' : 'gray'}
                    onClick={() => setDocumentType('financial-projection')}
                    leftIcon={<Text>💰</Text>}
                  >
                    Financial Projection (XLS/PDF)
                  </Button>
                </VStack>
              </FormControl>
            </VStack>
